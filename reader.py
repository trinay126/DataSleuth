# reader.py
# parser CSV-foramatted string data into structured data
...
def parse_csv(raw_text, delimiter=",", has_header=True):
    ...
    Args:
        ...
        delimiter: column seprator character (default comma)
        has_header: True if first line contains column names
    Retunrs :
        headers (list) : column name strings
        rows (list)    : list of dicts {col_name: value_strings}
        errors(list)   : description of any parsing problems
    """
    ...
    if has_header:
        ...
        headers = [h.strip().strip().strip('"').strip("'") for h in header_parts]
        data_lines = lines[1:]
    else:
    ...
    for line_num, line in enumerate(data_lines, start=2):
        ...
        if len(values) < len(headers):
            ...
            errors.append(f"Row {line_num}: extra coloumns trimmed")
