...
def check_whitespace(values, col):
    """Flag values that contain only whitespace."""
    ws_count = sum(1 for v in values
...
def check_range(values, col, lo, hi):
    ...
    if outside:
        return("ERROR", col, f"{len(outside)} values(s) outside expected range [{lo}, {hi}]")
...
def run_all_checks(headers, rows, range_rules=None, email_cols=None):
    """
    Runs the full quality check suite on all colums.
    ...
    Args:
        ...
        Returns:
            List of (severity, col_name, message)
    """
...
def count_alerts(alerts):
    """Return a count dict : {sevirity : count}"""
